...
async function suite() {
  ...
  console.log('\n=== 4b. 启用后旧会话已撤销，需携带验证码重新登录 ===');
  const reloginCode = totpAt(secret, 1);
  r = await c.req('POST', '/api/auth/login', { body: { username: 'admin', password: 'admin123', totp_code: reloginCode }, ticket, headers: loginHdr });
  assert('启用后携带验证码重新登录成功', r.status === 200 && r.body.data.token, `status=${r.status} ${r.body && r.body.message}`);
  token = r.body.data.token;
  authHeaders.Authorization = 'Bearer ' + token;

  console.log('\n=== 4c. 已启用时不允许直接轮换 2FA 密钥 ===');
  r = await c.req('POST', '/api/auth/2fa/setup', { body: { password: 'admin123' }, ticket, headers: authHeaders });
  assert('已启用时重新生成密钥被拒', r.status === 409, `status=${r.status} ${r.body && r.body.message}`);
